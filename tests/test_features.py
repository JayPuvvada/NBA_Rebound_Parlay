...
class PlayerStatsTest(unittest.TestCase):
    def test_total_only_fallback_uses_precutoff_observations_and_keeps_split_unknown(self):
        ...
        raw = pd.DataFrame([
            {'GAME_DATE': '2026-01-01', 'MIN': 30, 'REB': 12, 'MATCHUP': 'DEN @ LAL'},
            {'GAME_DATE': '2026-01-03', 'MIN': 30, 'REB': 99, 'MATCHUP': 'DEN @ LAL'},
        ])
        frame = loader._prepare_gamelog(raw, '2026-01-02')
        frame.attrs['total_rebounds_only'] = True
        loader.get_player_gamelog = Mock(return_value=frame)
        loader.get_common_player_info = Mock(return_value=pd.DataFrame([
            {'DISPLAY_FIRST_LAST': 'Nikola Jokic', 'TEAM_ID': 1610612743, 'POSITION': 'C'},
        ]))
        stats = FeatureEngineer(loader).get_player_stats(203999, 'LAL', '2026-01-02')
        self.assertFalse(stats['rebound_split_available'])
        self.assertEqual(stats['games_played'], 1)
        self.assertEqual(stats['reb_mean'], 12)
        self.assertAlmostEqual(stats['season_oreb_rate'] + stats['season_dreb_rate'], 12 / 30)
        self.assertNotIn('DREB', frame.columns)
...
class ProjectionSafetyTest(unittest.TestCase):
    ...
    def test_degraded_projection_inputs_are_diagnostic_only(self):
        loader = Mock()
        loader.get_injury_report_metadata.return_value = {
            'status': 'available', 'source': 'live_scrape',
            'fetched_at': '2026-09-04T12:00:00+00:00',
            'entry_count': 20, 'stale': False,
        }
        loader.get_data_source_metadata.return_value = {
            'status': 'degraded', 'source': 'espn',
            'limitations': ['player logs came from the fallback'],
        }

        safety = _projection_safety_context(loader, None)

        self.assertFalse(safety['prediction_eligible'])
        self.assertEqual(safety['data_sources']['source'], 'espn')
        self.assertIn('player logs came from the fallback', safety['limitations'])
        self.assertIn(
            'alternate or estimated data was used; projection is diagnostic-only',
            safety['limitations'],
        )

    def test_unknown_rebound_split_disables_split_dependent_adjustments(self):
        engineer = self._configured_engineer(Mock())
        engineer.get_player_stats.return_value['rebound_split_available'] = False
        result = engineer.compute_projection(1, 'LAL', as_of_date='2020-01-10')
        self.assertNotIn('error', result)
        self.assertEqual(result['components']['Miss Matchup'], 1.0)
        self.assertEqual(result['components']['Opp Rebound Environment'], 1.0)
