...
- `NBA_API_PROXY_VERIFY_SSL`: defaults to `true`; set to `false` only when an
  explicitly configured interception proxy requires it (for example,
  ScraperAPI's authenticated proxy endpoint).
...
NBA Stats remains the primary source. A connection timeout or blocking/server
HTTP failure pauses new NBA Stats calls for five minutes; the next uncached
request after that window tries the primary source again. An endpoint parsing
or argument error does not pause other NBA endpoints. This is request-driven
recovery, not a background health ping.

During outages, Player Lookup can use ESPN player identity, game totals, and
schedules. These results remain diagnostic-only, including when served from
cache. Fallback caches expire after five minutes. ESPN game logs do not supply
offensive/defensive rebound splits, so adjustments requiring those splits are
disabled; later season averages are never used to reconstruct historical
splits. Unavailable team matchup/rest data uses the disclosed neutral fallback.
Complete roster and advanced-stat fallbacks for Daily Edge are not provided.
