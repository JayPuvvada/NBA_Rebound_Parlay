...
class NBADataLoader:
    ...
    def get_player_gamelog(self, player_id, as_of=None):
        ...
        if failures:
            raise DataUnavailableError(
                f"player gamelog history is incomplete for {player_id}"
            ) from failures[-1]
    ...
    def get_common_player_info(self, player_id):
        ...
        key = f"common_info_{int(player_id)}"
        if self._get_from_cache(key) is not None:
            return self._get_from_cache(key)
    ...
    def _fetch_games_for_date(self, date_str):
        """Fetch and validate an uncached ScoreboardV2 slate."""
    ...
    @ttl_cache(seconds=900, cache_empty=True)
